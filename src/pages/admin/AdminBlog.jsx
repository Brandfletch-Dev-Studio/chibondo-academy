...
import { BookOpen, Plus, Edit2, Trash2, Eye, EyeOff, Loader2, Tag, X } from 'lucide-react';
...
const EMPTY_FORM = { title: '', slug: '', excerpt: '', content: '', cover_image: '', author_name: '', tags: [], status: 'draft' };
...
export default function AdminBlog() {
  ...
  const { data: posts = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.BlogPost.filter({}, '-created_date', 100),
  });

  const openNew = () => { setEditing(null); setForm(EMPTY_FORM); setTagInput(''); setOpen(true); };
  const openEdit = (p) => {
    setEditing(p);
    setForm({ title: p.title || '', slug: p.slug || '', excerpt: p.excerpt || '', content: p.content || '', cover_image: p.cover_image || '', author_name: p.author_name || '', tags: p.tags || [], status: p.status || 'draft' });
    setTagInput('');
    setOpen(true);
  };
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = { ...form, published_at: form.status === 'published' ? (editing?.published_at || new Date().toISOString()) : undefined };
  ...
  const toggleStatus = useMutation({
    mutationFn: ({ id, status }) => base44.entities.BlogPost.update(id, {
      status,
      ...(status === 'published' ? { published_at: new Date().toISOString() } : {}),
    }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['adminBlogPosts'] }),
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold flex items-center gap-2">
            <BookOpen className="w-6 h-6 text-accent" /> Blog Management
          </h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 border border-dashed border-border rounded-2xl">
          ...
          <Button onClick={openNew}><Plus className="w-4 h-4 mr-2" />Create First Post</Button>
        </div>
      ) : (
          ...
          <div className="space-y-4 mt-2">
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input value={form.slug} onChange={e => set('slug', slugify(e.target.value))} placeholder="post-url-slug" className="mt-1 font-mono text-sm" />
              </div>
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
