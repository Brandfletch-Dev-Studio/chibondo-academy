...
export default function AffiliateManagement() {
  ...
  // ── Data fetching ──────────────────────────────────────────────────────────
  const { data: allReferrals = [], isLoading: loadingRef } = useQuery({
    queryKey: ['admin-all-referrals'],
    queryFn: () => db.entities.Referral.list('-created_date', 500),
    staleTime: 0, refetchOnWindowFocus: true, refetchInterval: 30_000,
  });

  const { data: allPayouts = [], isLoading: loadingPay } = useQuery({
    queryKey: ['admin-all-payouts'],
    queryFn: () => db.entities.PayoutRequest.list('-created_date', 200),
    staleTime: 0, refetchOnWindowFocus: true, refetchInterval: 30_000,
  });

  const { data: affiliates = [] } = useQuery({
    queryKey: ['admin-affiliates'],
    queryFn: () => db.entities.User.filter({ role: 'affiliate' }, 'full_name', 200),
    staleTime: 60_000,
  });

  const { data: settingsRows = [] } = useQuery({
    queryKey: ['affiliateSettings'],
    queryFn: () => db.entities.PlatformSettings.filter({ key: 'affiliate_commission' }),
    staleTime: 60_000,
  });
